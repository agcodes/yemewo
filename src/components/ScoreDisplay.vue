<template>
  <div class="card border-0 mb-4 p-4 bg-highlight">
    ...
    <div class="text-center p-3">
      <div class="fs-2 bold mb-3">{{ game.roundPts }} / {{ game.nbRoundGames }}</div>

      <div class="mb-3">
        <span class="fs-5 badge bg-transparent border text-secondary border-secondary"
          >Temps : {{ game.elapsedTime }}</span
        >
      </div>

      <div class="bold text-muted mb-3">Round {{ game.nbRounds }}</div>

      <TransitionGroup
        name="round"
        tag="div"
        class="gap-2 mb-3 d-grid justify-content-center"
      ...
      >
        <span v-for="(round, index) in game.gameRounds" :key="round.index">
          <i
            v-if="round.level === 1"
            title="bronze"
            class="fs180 bi bi-trophy-fill level-bronze"
          ></i>
          <i
            v-else-if="round.level === 2"
            title="argent"
            class="fs180 bi bi-trophy-fill level-silver"
          ></i>
          <i
            v-else-if="round.level === 3"
            title="or"
            class="fs180 bi bi-trophy-fill level-gold"
          ></i>
