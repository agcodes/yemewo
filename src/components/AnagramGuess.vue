  ...
  <div v-else class="card border-0 mb-4 p-3">
    <div class="card-body">
      ...
      <div class="d-flex justify-content-center mt-1 gap-2 mb-3">
        <div
          v-for="(letter, index) in game.scrambledWord"
          :key="index"
          class="d-flex align-items-center text-black justify-content-center fs-4 shadow-primary"
          :style="{
            width: '65px',
            height: '65px',
      ...
      <div class="d-flex justify-content-center gap-2 mb-5">
        <div
          v-for="(letter, index) in game.currentWord"
          :key="index"
          class="d-flex align-items-center text-black justify-content-center fs-4 shadow-primary text-uppercase"
          :style="{
            width: '65px',
            height: '65px',
            backgroundColor: !letter.found ? 'red' : game.getLetterColor(letter.letter),
          }"
        >
          {{
            game.wordFound ? game.wordToGuess[index]?.toUpperCase() : letter.letter.toUpperCase()
          }}
        </div>
      </div>
      ...
      <div class="d-flex justify-content-center mb-5">
        <div style="max-width: 344px">
          ...
          <input
            ...
            @input="game.checkAnagramGuessOnInput"
            @keyup.enter="game.checkAnagramGuessOnInput"
          />
        </div>
      </div>
      <!-- Indice -->
